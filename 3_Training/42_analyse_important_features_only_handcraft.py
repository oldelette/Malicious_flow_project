...
def main():
	...
	model = lgb.Booster(model_file=Weight_dir + "20_new_lightgbm_handcraft_only.txt")
	feature_importance = pd.Series(model.feature_importance(), train_X.columns)
	feature_importance = feature_importance.sort_values(ascending=False)	
	important_feature_list = [feature_importance.index[i] for i in range(10)]	
	
	for id, feature_name in enumerate(important_feature_list):
		# Preprocess data
		...
		filename = analysis_dir + "42_" + feature_name + '.png' 
